...
object SqsClientBackend {
  ...
  /** Builds a raw AWS `SqsClient` (synchronous). Prefer to use `PureSqsClient` instead, where possible.
    */
  ...
  /** Builds a raw AWS `SqsAsyncClient`. Prefer to use `PureSqsClient` instead, where possible.
    */
